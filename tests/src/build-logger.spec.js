...
var buildLogger = require('../../src/build-logger');
...
describe('buildLogger', function () {
  ...
  it('must be obj', function () {
    newObj = buildLogger(originalObj, function (id, ts, event, payload) {
    });
    assert.instanceOf(newObj, Object);
  });

  it('must work with undefined obj', function () {
    newObj = buildLogger(undefined, function (id, ts, event, payload) {
    });
    assert(newObj);
  ...
  it('must log', function (done) {
    newObj = buildLogger(originalObj, 'name', function (name, id, ts, event, payload) {
